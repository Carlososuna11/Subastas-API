...
class CatalogoOrganizacionesView(APIView):

    @swagger_auto_schema(request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT, 
        properties={
            'id_organizaciones': openapi.Schema(type=openapi.TYPE_ARRAY,items=openapi.TYPE_NUMBER, description='los ids de las organizaciones')
        }
    ))
    @conectar
    def post(self, request,connection):
        ...
        cursor = connection.cursor(dictionary=True)
        id_organizaciones = request.data.get('id_organizaciones',[])

        id_organizaciones = self.request.data.get('id_organizaciones')
        moneda = ['id','nombre','denominacion','mintage','forma','metal','diametromm','canto',
        'pesogr','ano','motivo','acunacion','anverso','reverso','id_pais_divisa','id_pais','id_divisa',
        'imagen']
        divisa = ['id','id_pais','nombre']
        ...
        query_action = f"""SELECT 
                        ...
                        LEFT JOIN caj_paises as pais_reside
                        ON pais_reside.id = caj_coleccionistas.id_pais_reside
                        WHERE caj_Catalogo_Moneda_Tienda.id_organizacion IN ({','.join([str(i) for i in id_organizaciones])})
                        """
        #print(query_action)
        ...
        query_action = f"""SELECT 
                        ...
                        LEFT JOIN caj_paises as pais_reside
                        ON pais_reside.id = caj_coleccionistas.id_pais_reside
                        WHERE caj_Catalogo_Pintura_Tienda.id_organizacion IN ({','.join([str(i) for i in id_organizaciones])})
                        """
        #print(query_action)
        cursor.execute(query_action)
        ...
        #     print(dato)
        catalogoPinturas = [Pintura.model(**dato) for dato in catalogo]
        for i in catalogoPinturas:
            i.to_representation()
        for i in catalogoMonedas:
            i.to_representation()
        catalogoPinturas.sort(key=lambda x: x.nur)
        pinturas = get_json(catalogoPinturas)
        monedas = get_json(catalogoMonedas)
        return Response({'pinturas':pinturas,'monedas':monedas})
